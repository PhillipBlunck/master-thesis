...
#############################################################################
##                                                                         ##
## trainmlptemp.py: Train an MLP-Network and display the loss curve and    ##
##                  other metrics.                                         ##
##                                                                         ##
...
def plottrainingresult(model, logger, signals, prediction, test_y,
    ...
    matplotlib.pyplot.title("Temperature model evaluation with test set")
...
def plotvalidationsetresult(logger, signals, prediction, test_x, test_y,
    ...
    matplotlib.pyplot.title(
            "Temperature model evaluation with validation set"
    )
    #matplotlib.pyplot.xlabel("ith sample")
    ...
    matplotlib.pyplot.plot(test_x[:,2])
    matplotlib.pyplot.grid(True)
...
def training(stdin):
    try:
        ...
        for logger in sorted(loggers):
            ...
            # combine into array of features current, temperature, SoC
            if DEVELOP:
                X, y = logdata[:500000:STEP, [3,4,5]], \
                    logdata[:500000:STEP, 2]
            else:
                X, y = logdata[::STEP, [3,4,5]], logdata[::STEP, 2]
            ...
            if DEVELOP:
                Xt, yt = logdata[625000::STEP, [3,4,5]], \
                    logdata[625000::STEP, 2]
            # normalize data (Min-max feature scaling)
            # train set
            X_train_current = scale_minmax(
                X_train[:, 0], MIN_CURRENT, MAX_CURRENT
            )
            X_train_temp = scale_minmax(X_train[:, 1], MIN_TEMP, MAX_TEMP)
            X_train_soc = scale_minmax(X_train[:, 2], MIN_SOC, MAX_SOC)
            X_train_norm = numpy.vstack(
                (X_train_current, X_train_temp, X_train_soc)
            ).T
            # test set
            X_test_current = scale_minmax(X_test[:, 0], MIN_CURRENT,
                MAX_CURRENT
            )
            X_test_temp = scale_minmax(X_test[:, 1], MIN_TEMP, MAX_TEMP)
            X_test_soc = scale_minmax(X_test[:, 2], MIN_SOC, MAX_SOC)
            ...
            if DEVELOP:
                ...
                Xt_temp = scale_minmax(Xt[:, 1], MIN_TEMP, MAX_TEMP)
                Xt_soc = scale_minmax(Xt[:, 2], MIN_SOC, MAX_SOC)
                X_test_sor_norm = numpy.vstack(
                    (Xt_current, Xt_temp, Xt_soc)
                ).T
            # Set up the model
            regr = MLPRegressor(#20,15
        ...
        if SAVE_MODEL:
            ...
            dump(regr, f'models/mlptemp-{datahash}.joblib')
